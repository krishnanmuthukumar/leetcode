...
public class RomanToInteger {
	...
	public int romanToInt(String s) {
		...
		Map<String, Integer> romanSymbolMap = new HashMap<String, Integer>();
		...
		Integer number = 0, previous = 0;
		char[] charArray = s.toCharArray();
		int[] intArray = new int[charArray.length];
		int index = 0;
		for (char c : charArray) {
			if (romanSymbolMap.get(String.valueOf(c)) != null) {
				intArray[index] = romanSymbolMap.get(String.valueOf(c));
				++index;
			}
		}

		for (int i = 0; i < intArray.length; i++) {
			if (previous != 0 && intArray[i] > previous) {
				number = number - previous;
				number = number + intArray[i] - previous;
			} else {
